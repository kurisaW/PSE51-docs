...
参数 `argN` 是函数定义中可变参数列表中最右边参数的标识符（即 `...` 之��的那一个）。如果参数 `argN` 使用 **register** 存储类别声明、具有函数类型或数组类型，或者具有与默认参数提升后所得类型不兼容的类型，则行为是未定义的。
...
如果没有实际的下一个参数，或者 `type` 与实际下一个参数的类型（根据默认��数提升进行提升后）不兼容，则行为是未定义的，但以下情况除外：
