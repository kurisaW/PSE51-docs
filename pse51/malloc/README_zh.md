...
通过连续调用 `malloc()` 分配的存储的顺序和连续性是未指定的。如果分配成功，返回的指针应适当对齐，以便可以将其赋给具有基本对齐要求的任何类型对象的指针，然后用于在分配的空间中访问此类对象（直到空间被��式释放或重新分配）。每次此类分配都会产生一个与任何其他对象不重叠的对象指针。返回的指针指向分配空间的开头（最低字节地址）。如果无法分配空间，应返回空指针。如果请求的空间大小为 0，其行为是实现定义的：要么返回空指针，要么行为如同大小为某个非零值，除非如果使用返回的指针访问对象，则行为是未定义的。
