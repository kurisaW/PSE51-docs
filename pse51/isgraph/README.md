...
The `isgraph()` [CX] and `isgraph_l()` functions shall test whether `c` is a character of class **graph** in the current locale, [CX] or in the locale represented by `locale`, respectively; see XBD [7. Locale](https://pubs.opengroup.org/onlinepubs/9799919799/basedefs/V1_chap07.html#tag_07).
...
[CX] The behavior is undefined if the `locale` argument to `isgraph_l()` is the special locale object LC_GLOBAL_LOCALE or is not a valid locale object handle.
