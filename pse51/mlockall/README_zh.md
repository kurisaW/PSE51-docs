...
`mlockall()` 函数应使进程地址空间映射的所有页面在内存中保持驻留，直到被解锁、进程退出或 `exec` 执行另一个进程映像为止。`flags` 参数决定���锁定的页面是当前由进程地址空间映射的页面、将来要映射的页面，还是两者兼有。`flags` 参数由以下一个或多个符号常量的按位包含 OR 构成，这些常量定义在 `<sys/mman.h>` 中：
